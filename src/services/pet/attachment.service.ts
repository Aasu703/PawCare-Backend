...
export class AttachmentService {
    ...
    async createAttachment(data: CreateAttachmentDto) {
        ...
        const record = await this.healthRecordRepository.getHealthRecordById(data.healthRecordId);
        if (!record) {
            throw new HttpError(404, "Health record not found");
        }
        const attachment = await this.attachmentRepository.createAttachment(data);
        await this.healthRecordRepository.incrementAttachmentsCount(data.healthRecordId);
        return attachment;
    }
    ...
    async getAttachmentsByHealthRecordId(healthRecordId: string) {
        return this.attachmentRepository.getAttachmentsByHealthRecordId(healthRecordId);
    }

    async updateAttachment(id: string, data: UpdateAttachmentDto) {
        const updated = await this.attachmentRepository.updateAttachmentById(id, data);
        if (!updated) {
            throw new HttpError(404, "Attachment not found");
        }
        return updated;
    ...
    async deleteAttachment(id: string) {
        const existing = await this.attachmentRepository.getAttachmentById(id);
        if (!existing) {
            throw new HttpError(404, "Attachment not found");
        }
        const deleted = await this.attachmentRepository.deleteAttachmentById(id);
        if (deleted && existing.healthRecordId) {
            await this.healthRecordRepository.decrementAttachmentsCount(existing.healthRecordId);
        }
        return deleted;
    }
}
